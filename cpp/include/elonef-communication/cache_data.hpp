...
namespace Elonef {
    ...
    struct CacheHandlerData {
        ...
        CacheHandlerData(const std::vector<Key> users, Handler* handler, std::function<std::vector<std::pair<Key, T>>(Handler* handler, CryptoPP::ByteQueue& queue)>& decoder);
...
template<typename Key, typename Handler, typename T>
inline Elonef::CacheHandlerData<Key, Handler, T>::CacheHandlerData(const std::vector<Key> _users, Handler* _handler, std::function<std::vector<std::pair<Key, T>>(Handler* handler, CryptoPP::ByteQueue& queue)>& _decoder) : 
